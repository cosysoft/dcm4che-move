 ...
 * 
 */
public class MoveSCUFacade extends Device {
	...
	private static enum InformationModel {
		PatientRoot(UID.PatientRootQueryRetrieveInformationModelMOVE, "STUDY"), StudyRoot(
				UID.StudyRootQueryRetrieveInformationModelMOVE, "STUDY"), PatientStudyOnly(
				UID.PatientStudyOnlyQueryRetrieveInformationModelMOVERetired,
				"STUDY"), CompositeInstanceRoot(
				UID.CompositeInstanceRootRetrieveMOVE, "IMAGE"), HangingProtocol(
				UID.HangingProtocolInformationModelMOVE, null), ColorPalette(
				UID.ColorPaletteInformationModelMOVE, null);
	...
	private static ResourceBundle rb = ResourceBundle
			.getBundle("org.dcm4che.tool.movescu.messages");

	private static final int[] DEF_IN_FILTER = { Tag.SOPInstanceUID,
	...
	public MoveSCUFacade() throws IOException {
		super("movescu");
		addConnection(conn);
	...
	private static CommandLine parseComandLine(String[] args)
		...
		return CLIUtils.parseComandLine(args, opts, rb, MoveSCUFacade.class);
	}
	...
	private static void addDestinationOption(Options opts) {
		opts.addOption(OptionBuilder.withLongOpt("dest").hasArg()
				.withArgName("aet").withDescription(rb.getString("dest"))
				.create());
	...
	private static void addKeyOptions(Options opts) {
		opts.addOption(OptionBuilder.hasArgs().withArgName("attr=value")
	...
	public void execute(String[] args) {
		try {
			...
			MoveSCUFacade main = new MoveSCUFacade();
	...
	private static void configureServiceClass(MoveSCUFacade main, CommandLine cl)
			throws ParseException {
	...
	private static void configureKeys(MoveSCUFacade main, CommandLine cl) {
		if (cl.hasOption("m")) {
			String[] keys = cl.getOptionValues("m");
			for (int i = 1; i < keys.length; i++, i++)
				main.addKey(CLIUtils.toTag(keys[i - 1]),
	...
	private void retrieve(Attributes keys) throws IOException,
